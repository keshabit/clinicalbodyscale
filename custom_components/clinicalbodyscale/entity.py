"""ClinicalBodyScale entity module."""
...
# Import CONF_CALCULATION_MODE to read the selected algorithm mode
from .const import CONF_CALCULATION_MODE, DOMAIN, VERSION
...
class BodyScaleBaseEntity(Entity):
    ...
    def __init__(
        ...
        # Determine a friendly model string based on the configuration mode
        algo_mode = self._handler.config.get(CONF_CALCULATION_MODE, "xiaomi")
        model_name = f"Scale Profile ({algo_mode.upper()} mode)"

        self._attr_device_info = DeviceInfo(
            entry_type=DeviceEntryType.SERVICE,
            name=self._handler.config[CONF_NAME],
            model=model_name,  # Displays the algorithm mode in the HA UI
            sw_version=VERSION,
