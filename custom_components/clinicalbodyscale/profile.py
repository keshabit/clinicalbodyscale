"""Profile identification strategies for ClinicalBodyScal.
...
class ProfileIdFilter(ProfileFilter):
    ...
    def accepts(
        ...
        except (ValueError, TypeError):  # Fixed Python 3 Syntax Error here
            _LOGGER.debug(
...
class NearestWeightFilter(ProfileFilter):
    ...
    def accepts(
        ...
        best_distance = min(d for d, _name in candidates)
        ...
        tied_names = sorted(
            name for d, name in candidates if d == best_distance
        )
